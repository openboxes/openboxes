...
@Unroll
class InstantFormatterSpec extends Specification {
    ...
    void setup() {
        ...
        instantFormatter.datePatternLocalizer = datePatternLocalizerStub
        localeManagerStub = Stub(LocaleManager)
        instantFormatter.localeManager = localeManagerStub
        sessionManagerStub = Stub(SessionManager)
        instantFormatter.sessionManager = sessionManagerStub
    }

    void 'format can handle null inputs gracefully'() {
        expect:
        instantFormatter.format(null) == null
    }
    ...
    void 'format succeeds when given no format options'() {
        ...
        expect:
        instantFormatter.format(MIDNIGHT_JAN_1ST_2000_UTC) == '01/Jan/2000 00:00:00'
    }
    ...
    void 'format succeeds when given a display format for scenario: #scenario'() {
        ...
        expect:
        instantFormatter.format(MIDNIGHT_JAN_1ST_2000_UTC, context) == expectedResult
        ...
        Locale.ENGLISH | DateDisplayFormat.CSV  | 'Z'         || '2000-01-01'                | 'CSV format localization (english) UTC'
        Locale.ITALIAN | DateDisplayFormat.CSV  | 'Z'         || '2000-01-01'                | 'CSV format localization (non-english) UTC'
        ...
        Locale.ENGLISH | DateDisplayFormat.CSV  | 'GMT+01:00' || '2000-01-01'                | 'CSV format localization (english) UTC+1'
        Locale.ITALIAN | DateDisplayFormat.CSV  | 'GMT+01:00' || '2000-01-01'                | 'CSV format localization (non-english) UTC+1'
