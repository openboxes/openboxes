...
class ApiControllerFunctionalSpec extends Specification {
    ...
    def setup() {
        ...
        Organization organization = Organization.findOrCreateWhere([code: "MYC", name: "My Company", partyType: orgPartyType])
        LocationType defaultLocationType = LocationType.findByLocationTypeCode(LocationTypeCode.DEPOT)
        location = Location.findOrCreateWhere([locationNumber: "MAIN", name: "Main Warehouse 2", locationType: defaultLocationType, organization: organization])
        location.save()
