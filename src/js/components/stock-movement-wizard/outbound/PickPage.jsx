...
class PickPage extends Component {
  ...
  loadMoreRows({ startIndex, stopIndex }) {
    const url = `/api/stockMovements/${this.state.values.stockMovementId}/stockMovementItems?offset=${startIndex}&max=${stopIndex - startIndex > 0 ? stopIndex - startIndex : 1}&stepNumber=4`;
    apiClient.get(url)
      .then((response) => {
        this.setPickPageItems(response);
      });
        ...
        this.setState({
          values: {
            ...
            pickPageItems: _.map(parseResponse(pickPageItems), item =>
              this.checkaForInitialPicksChanges(item)),
          },
          sorted: false,
