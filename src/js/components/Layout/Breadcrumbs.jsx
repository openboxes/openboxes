    ...
    const listToReturn = listItemFormatted.map((name, id) =>
      (name === 'Openboxes' || name === '' ? null : (
        <a key={`item-${id}`} href={window.location.pathname.split('/', id + 1).join('/')} className="item-breadcrumbs">
          {name}
          <img className="item-breadcrumbs" alt="/" src={`${window.CONTEXT_PATH}/static/images/bc_separator.png`} />
        </a>
      )));

    return (
      <div className="breadcrumbs-container d-flex">
        <a className="item-breadcrumbs" href={window.CONTEXT_PATH || '/'}>
          <img alt="Breadcrumbs" src={`${window.CONTEXT_PATH}/static/images/skin/house.png`} />
        </a>
        <img className="item-breadcrumbs" alt="/" src={`${window.CONTEXT_PATH}/static/images/bc_separator.png`} />
        <a
          role="button"
