...
(
SELECT CRC32(CONCAT(cycle_count_metadata_tmp.inventory_id, cycle_count_metadata_tmp.product_id))                                          as id,
       ...
       COALESCE(inventory_level.cycle_count_frequency_days, cycle_count_frequency.frequency)                        as frequency,
       datediff(now(), cycle_count_metadata_tmp.date_counted)                                                         as days_since_last_count,
       date_add(cycle_count_metadata_tmp.date_counted, INTERVAL COALESCE(inventory_level.cycle_count_frequency_days, cycle_count_frequency.frequency)
                DAY)                                                                                                  as date_expected,
       datediff(date_add(cycle_count_metadata_tmp.date_counted, INTERVAL COALESCE(inventory_level.cycle_count_frequency_days, cycle_count_frequency.frequency) DAY),
...
FROM (SELECT product_count_history.inventory_id as inventory_id,
      ...
      FROM product_count_history
          ...
          LEFT OUTER JOIN inventory_level ON inventory_level.inventory_id = cycle_count_metadata_tmp.inventory_id
                                           AND inventory_level.product_id = cycle_count_metadata_tmp.product_id
);