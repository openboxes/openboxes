CREATE OR REPLACE VIEW order_item_status AS
    ...
    FROM (
        ...
        WHERE `order`.order_type_code = 'PURCHASE_ORDER' AND `order`.status NOT IN ('COMPLETED', 'CANCELLED') AND order_item.order_item_status_code != 'CANCELLED'
...
CREATE OR REPLACE VIEW order_receipt_status AS
    ...
    FROM (
        ...
        WHERE `order`.order_type_code = 'PURCHASE_ORDER' AND `order`.status NOT IN ('COMPLETED', 'CANCELLED') AND order_item.order_item_status_code != 'CANCELLED' AND shipment.current_status = 'RECEIVED'
...
CREATE OR REPLACE VIEW order_payment_status_from_shipments AS
    ...
    FROM (
        ...
        WHERE `order`.order_type_code = 'PURCHASE_ORDER' AND `order`.status NOT IN ('COMPLETED', 'CANCELLED') AND order_item.order_item_status_code != 'CANCELLED' AND invoice.date_submitted IS NOT NULL
