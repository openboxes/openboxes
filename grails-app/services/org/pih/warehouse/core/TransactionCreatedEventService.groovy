            ...
            switch (transaction.transactionType.transactionCode) {
                ...
                case TransactionCode.CREDIT:
                    if (transaction.cycleCount || transaction.transactionSource?.cycleCount) {
                        // This case is handled by the CycleCountCompletedEvent, so we can skip it here
                        break
                    }
                    ...
                    boolean webhooksEnabled = transaction.inventory.warehouse.supports(ActivityCode.ENABLE_WEBHOOKS)
                    if (!webhooksEnabled) {
                        log.info "Location ${transaction.inventory.warehouse} does not support activity code ${ActivityCode.ENABLE_WEBHOOKS}"
                        break
                    }
                    ...
                    transaction.associatedProducts?.each { String productId ->
                        try {
                            ...
                            log.error "Error publishing inventory adjustment event for product with id: $productId", e
                        }
                    }
                    break
