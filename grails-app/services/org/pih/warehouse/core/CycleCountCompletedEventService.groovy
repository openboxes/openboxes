...
class CycleCountCompletedEventService {
    ...
    void onCycleCountEvent(CycleCountCompletedEvent event) {
        CycleCount.withNewTransaction {
            ...
            boolean webhooksEnabled = cycleCount.facility.supports(ActivityCode.ENABLE_WEBHOOKS)
            if (!webhooksEnabled) {
                log.info "Location ${cycleCount.facility} does not support activity code ${ActivityCode.ENABLE_WEBHOOKS}"
                return
            }
