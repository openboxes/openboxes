...
class ShipmentService {
    ...
    List<Shipment> getShipmentsByLocation(Location origin, Location destination, ShipmentStatusCode shipmentStatusCode, List<RequisitionStatus> requisitionStatuses = []) {
        return Shipment.withCriteria {
            ...
            order("expectedShippingDate", "asc")
        }
    }
    ...
    boolean validateReturnShipmentItem(ShipmentItem shipmentItem) {
        ...
        def origin = Location.get(shipmentItem?.shipment?.origin?.id)
        def quantityAvailableToReturn = productAvailabilityService.getQuantityNotPickedInBinLocation(shipmentItem.inventoryItem, origin, shipmentItem.binLocation)
