...
class ShipmentService {
    ...
    List<Shipment> getShippedShipmentsByDestination(Location destination) {
        return getShipmentsByCriteria(null, destination, ShipmentStatusCode.SHIPPED, [])
    }
    ...
    boolean validateShipmentItem(ShipmentItem shipmentItem) {
        ...
        if (origin.requiresOutboundQuantityValidation()) {
            ...
            // quantities below 0. Orders that do not permit negative inventory keep the ordinary checks.
            if (shipmentItem.shipment?.requisition?.negativeInventoryAllowed &&
