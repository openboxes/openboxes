...
class AzureAuthController {
    ...
    def config = {
        render ([data:azureAuthService.config] as JSON)
    }

    def callback = {
        log.info "callback: " + params
        if (params.code) {
            def token = azureAuthService.getToken(params.code)
    ...
    def login = {
        redirect(url: azureAuthService.authenticationUrl)
    }
