...
class StockMovementController {
    ...
    def containerDialog = {
        ...
        def packageType
        def packageTypes = grailsApplication.config.openboxes.shipping.packageTypes
        if (packageTypes) {
            packageType = packageTypes.find { it.id == params.packageType }
        }
        def container = params.id ? Container.get(params.id) : new Container()
        if (!container) {
            flash.message = "${warehouse.message(code: 'default.not.found.message', args: [warehouse.message(code: 'container.label', default: 'Container'), params.id])}"
        }

        // New container based on package type
        if (!container.id && packageType) {
            container.name = packageType.name
            container.containerNumber = packageType.id + identifierService.generateIdentifier(10)
            container.containerType = ContainerType.findByNameIlike("${packageType.containerType}%")
            container.weight = packageType.weight
            container.weightUnits = packageType.weightUnits
            container.height = packageType.height
            container.width = packageType.width
            container.length = packageType.length
            container.volumeUnits = packageType.volumeUnits
        }
        log.info "${container.containerType}"

        render(template: "containerDialog", model: [container:container, stockMovement:stockMovement])
    }
    ...
    def saveContainer = {
        ...
        if (shipment && container.validate()) {
            ...
            flash.errors = container.errors
        }

        redirect(controller: "stockMovement", action: "show", id: params.stockMovement.id)
    }
    ...
    def deleteContainer = {
        ...
        if (shipment && container) {
            ...
            container.delete()
            shipment.save()
            flash.message = "Container was successfully deleted"
        }
        redirect(controller: "stockMovement", action: "show", id: params.stockMovement.id)
    }
