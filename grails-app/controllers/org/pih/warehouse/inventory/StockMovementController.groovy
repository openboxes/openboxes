...
class StockMovementController {
    ...
    def packingList() {
        def stockMovement = getStockMovement(params.id)
        Map<String, Requisition> requisitionsByReference =
                backorderService.resolveRequisitionsByReference(stockMovement?.shipment?.uniqueBackorderReferences)
        render(template: "packingList", model: [stockMovement: stockMovement, requisitionsByReference: requisitionsByReference])
    }
    ...
    def clearPicklist() {
        try {
            ...
            flash.error = "Error while clearing picklist for stock movement: ${e.message}"
            redirect(action: "show", id: params.id)
        }
    }
