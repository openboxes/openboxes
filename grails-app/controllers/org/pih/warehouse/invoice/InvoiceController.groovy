...
class InvoiceController {
    ...
    def downloadTemplateDocument = {
        Document documentInstance = Document.get(params.document.id)
        Invoice invoiceInstance = Invoice.get(params.invoice.id)
        if (documentInstance && invoiceInstance) {
            try {
                ByteArrayOutputStream outputStream = new ByteArrayOutputStream()
                documentTemplateService.renderInvoiceTemplate(documentInstance, invoiceInstance, outputStream)
                response.setHeader("Content-disposition",
                    "attachment; filename=\"${documentInstance.filename}\"-${invoiceInstance.invoiceNumber}.${documentInstance.extension}")
                response.setContentType(documentInstance.contentType)
                outputStream.writeTo(response.outputStream)
                response.outputStream.flush()
            } catch (Exception e) {
                log.error("Unable to render document template ${documentInstance.name} for invoice ${invoiceInstance?.id}", e)
                throw e
            }
        }
    }
