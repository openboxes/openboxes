...
class Shipment implements Comparable, Serializable {
    ...
    ShipmentStatusCode getDisplayStatus() {
        Boolean allContainersLoaded = containers?.every { it.containerStatus in [ContainerStatus.LOADED] }
        Boolean anyContainerLoading = containers?.any { it.containerStatus in [ContainerStatus.LOADED, ContainerStatus.LOADING] }
        Boolean allContainersPacked = containers?.every { it.containerStatus in [ContainerStatus.PACKED] }
        Boolean anyContainerPacking = containers?.any {it.containerStatus in [ContainerStatus.PACKED, ContainerStatus.PACKING] }
