            ...
            <g:each var="container" in="${shipmentInstance?.containers + null}">
                ...
                <g:if test="${shipmentItems}">
                    <g:each var="shipmentItem" in="${shipmentItems}" status="i">
                        <g:set var="unitOfMeasure" value="${shipmentItem?.inventoryItem?.product?.unitOfMeasure?:warehouse.message(code:'default.each.label')}"/>
                        <tr class="prop ${count++ % 2 == 0?'odd':'even'} ${shipmentItem?.hasRecalledLot?'recalled':''} shipmentItem">
                            ...
                            <td class="product">
                                <g:link controller="mobile" action="productDetails" id="${shipmentItem?.inventoryItem?.product?.id}">
