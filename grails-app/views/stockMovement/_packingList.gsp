...
<div id="packingList" class="dialog">
    <g:if test="${shipmentInstance?.containers + null}">
        <g:set var="count" value="${0}"/>
        <g:each var="container" in="${(shipmentInstance?.containers + null).sort()}">
            <g:set var="shipmentItems" value="${shipmentInstance.findShipmentItemsByContainer(container)}"/>
            <g:if test="${!container || !shipmentItems.empty}">
                <div class="box">
                    <h2 style="height: 60px">
                        <div class="left middle">
                            <g:if test="${container}">
                                <img src="${createLinkTo(dir:'images/icons/silk',file:'package.png')}" alt="contents" />
                                &nbsp;${container?.name}
                                <g:if test="${container?.containerStatus}">
                                    <span class="box-status info small" style="height: 20px">
                                        <format:metadata obj="${container?.containerStatus}"/>
                                    </span>
                                </g:if>
                            </g:if>
                            <g:else>
                        ...
                        <g:if test="${container?.containerNumber}">
                            <div class="box-barcode right">
                                <g:displayBarcode data="${container?.containerNumber}" height="20" showData="${true}"/>
                            </div>
                        </g:if>
                    </h2>
                    <table>
                        ...
                        <g:if test="${shipmentItems.empty}">
                            <tr>
                                <td colspan="11" class="center empty fade">
                                    <g:message code="shipment.noShipmentItems.message" default="No shipment items"/>
                                </td>
                            </tr>
                        </g:if>
                        <g:if test="${shipmentItems}">
                            ...
                            <tr class="prop ${count++ % 2 == 0?'odd':'even'} ${shipmentItem?.hasRecalledLot?'recalled':''} shipmentItem">
                                    ...
                                    <td>
                                        ...
                                        <g:else>
                                            ${shipmentItem?.inventoryItem?.lotNumber?:warehouse.message(code: 'default.label')}
                                        </g:else>
                                    </td>
