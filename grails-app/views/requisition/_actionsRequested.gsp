<g:if test="${requestInstance?.id }">
	<div class="actions" style="min-width: 200px;">
		...
		<div class="action-menu-item">
			<g:link controller="requisition" action="addComment" id="${requestInstance?.id}">
		...
		<div class="action-menu-item">
			<g:link controller="requisition" action="addDocument" id="${requestInstance?.id}">
		...
		<g:if test="${session?.warehouse?.id == requestInstance?.origin?.id }">
			...
			<div class="action-menu-item">
				<g:link controller="requisition" action="showPicklist" id="${requestInstance?.id}">
		...
		<g:if test="${session?.warehouse?.id == requestInstance?.destination?.id }">
			<g:if test="${requestInstance?.status == org.pih.warehouse.requisition.RequisitionStatus.FULFILLED }">
				<div class="action-menu-item">
			...
			<div class="action-menu-item">
				<g:link controller="requisition" action="withdraw" id="${requestInstance?.id}" onclick="alert('${warehouse.message(code: 'default.button.notSupported.message', default: 'This feature is not currently supported.')}'); return false;">
			...
			<div class="action-menu-item">
				<g:link controller="requisition" action="delete" id="${requestInstance?.id}" onclick="return confirm('${warehouse.message(code: 'default.button.delete.confirm.message', default: 'Are you sure?')}');">
