...
<table id="requisition-items">
    <tr class="theader">
        ...
        <th>${warehouse.message(code: 'default.comments.label')}</th>
        <th>${warehouse.message(code: 'requisition.receivedBy.label', default: 'Received by')}</th>
    </tr>

    <g:each in="${requisition?.requisitionItems.findAll { !it.isCanceled()&&!it.isChanged() } }" status="i" var="requisitionItem">
        <g:if test="${picklist}">
